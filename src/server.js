...
app.post('/api/admin/customers/:id/delete-confirm',auth,csrf,requireRole('admin'),customerDeleteLimiter,async(req,res,next)=>{
  ...
  try{
    ...
    await c.query('DELETE FROM customer_verification_challenges WHERE customer_id=$1',[id]);
    await c.query('DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE customer_id=$1)',[id]);
