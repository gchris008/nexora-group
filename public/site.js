...
async function loadFeatured(){
  ...
  try{const r=await fetch('/api/products/featured');if(!r.ok)throw new Error();const items=await r.json();el.innerHTML=items.length?items.map(productCard).join(''):'<p class="empty-state">Aucun produit en vogue pour le moment.</p>';}catch{el.innerHTML='';}
}
